...
class RobertaModel(RobertaPreTrainedModel):
    ...
    def forward(
        ...
        fused_embedding = self.MAG(encoder_outputs[0], visual, acoustic)
        if mode == 'test':
            cls_feat = fused_embedding[:, 0, :]
            pdist = nn.PairwiseDistance(p=1)
            for i in range(fused_embedding.shape[0]):
                # score = list(map(lambda x: x @ cls_feat[i], cat_feature))
                score = [pdist(torch.unsqueeze(x, 0), torch.unsqueeze(cls_feat[i], 0)) for x in
                         cat_feature]
                # score = [torch.cosine_similarity(x, cls_feat[i], dim=0) for x in cat_feature]
                fused_embedding[i, 0, :] = 0.1 * fused_embedding[i, 0, :] + 0.9 * cat_feature[score.index(min(score))]

        sequence_output = fused_embedding
...
class RobertaForSequenceClassification(RobertaPreTrainedModel):
    def __init__(self, config, multimodal_config):
        ...
        self.roberta = RobertaModel(config, multimodal_config, add_pooling_layer=False)
    ...
    def forward(
        ...
        if mode == 'train':
            new_cat_feature = aug(outputs[0], label_ids, mode='aug')
            feature = outputs[0]
